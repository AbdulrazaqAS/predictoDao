...
contract RewardManager {
    ...
    function distributeReward(uint256 _predId, uint256 _mtxId) external {
        ...
        // require(quesManager.isValidPredictionId(_predId), "Invalid prediction id");
        
        (, , uint256 reward, , QuestionManager.ValidAnswer memory validAns) = quesManager.predictions(_predId);
        require(reward > 0, "No reward assigned for this prediction.");
        ...
        quesManager.markRewardDistributed(_predId);
        lockedAmount -= reward;
        ...
        if (validAnswerIdx != -1) {  // -1 when the valid answer in not in the array
            ...
            address[] memory winners = quesManager.getAnswerVoters(_predId, uint256(validAnswerIdx));
